...
export default async function ChangeOrderPrintPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: co } = await supabase.from('change_orders').select('*,contracts(contract_number,client_name,client_address),projects(project_number,project_name,project_address),change_order_items(*)').eq('id', id).maybeSingle()
  if (!co) notFound()
  ...
  return <main className="contract-document"><article className="contract-sheet change-order-sheet">
    ...
    {items.length ? <section><h2>LABOR &amp; MATERIAL BREAKDOWN</h2><table className="contract-payment-table"><thead><tr><th>Type</th><th>Description</th><th>Qty</th><th>Unit</th><th>Unit Price</th><th>Total</th></tr></thead><tbody>{items.map((item: any) => <tr key={item.id}><td>{String(item.category || '').replace(/^./, (c: string) => c.toUpperCase())}</td><td>{item.description}</td><td>{Number(item.quantity || 0).toLocaleString('en-US')}</td><td>{item.unit || 'LS'}</td><td>{money(item.unit_price)}</td><td>{money(item.line_total)}</td></tr>)}</tbody></table></section> : null}

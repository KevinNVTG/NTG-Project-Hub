...
export default async function ChangeOrderPrintPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="contract-document"><article className="contract-sheet change-order-sheet">
    ...
    {co.status === 'void' ? <section><h2>VOID NOTICE</h2><p>This change order has been voided and has no current financial effect on the linked contract. Reason: {co.void_reason || 'No reason entered.'}</p></section> : null}
    {co.corrected_at ? <section><h2>CORRECTION RECORD</h2><p>This change order was corrected after approval. {co.correction_note || ''}</p></section> : null}
    ...
    <section><h2>CONTRACT PRICE REVISION</h2><table className="contract-payment-table change-order-price-table"><tbody><tr><td>Contract Price Before This Change Order</td><td>{money(co.original_contract_price)}</td></tr><tr><td>Change Order Amount</td><td>{money(co.amount)}</td></tr><tr><td><strong>{co.status === 'void' ? 'Contract Price After Voiding This Change Order' : 'Revised Contract Price'}</strong></td><td><strong>{money(co.revised_contract_price)}</strong></td></tr></tbody></table></section>
    ...
    {co.status !== 'void' ? <section className="contract-acceptance"><h2>ACCEPTANCE AND SIGNATURES</h2><p>By signing below, the Client and Contractor accept this Change Order and agree to the revised scope, price, schedule impact, and payment terms stated above.</p><div className="contract-signatures"><div><span>Client Signature</span></div><div><span>Date</span></div><div className="signature-name"><strong>Print Name: {contract?.client_name || '________________________'}</strong></div><div></div><div><span>Contractor&apos;s Signature</span></div><div><span>Date</span></div><div className="signature-name"><strong>Print Name: Kevin Melendez | Nevada Tile &amp; Granite</strong></div><div></div></div></section> : null}

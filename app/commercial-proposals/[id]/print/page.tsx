...
export default async function ProposalPrint({params}:{params:Promise<{id:string}>}){
  ...
  const {data:p}=await supabase.from('commercial_proposals').select('*,projects(project_number,project_name,project_address),customers(first_name,last_name,company_name,email,phone,billing_address),commercial_proposal_csi_sections(*),commercial_proposal_items(*)').eq('id',id).maybeSingle()
  if(!p)notFound()
  ...
  const base=baseItems.reduce((s:number,i:any)=>s+Number(i.quantity)*Number(i.unit_price),0)
  ...
  return <main className="commercial-proposal-print">
    ...
    {sections.map((s:any,index:number)=>{
      const sectionItems=baseItems.filter((i:any)=>i.csi_section_id===s.id)
      ...
      return <section key={s.id} className="proposal-print-section proposal-trade-sheet" style={separateSheets&&index>0?{breakBefore:'page'}:undefined}>
        ...
        <h3>{s.csi_title} — Pricing Schedule</h3>
        <table className="proposal-price-table"><thead><tr><th>Description</th><th>Qty</th><th>Unit</th><th>Unit Price</th><th>Total</th></tr></thead><tbody>{sectionItems.length?sectionItems.map((i:any)=><tr key={i.id}><td>{i.description}</td><td>{Number(i.quantity).toLocaleString()}</td><td>{i.unit}</td><td>{money(i.unit_price)}</td><td>{money(Number(i.quantity)*Number(i.unit_price))}</td></tr>):<tr><td colSpan={5}>No pricing items assigned to this scope.</td></tr>}</tbody><tfoot><tr><td colSpan={4}>{s.csi_code} SUBTOTAL</td><td>{money(subtotal)}</td></tr></tfoot></table>
      </section>
    })}

    {baseItems.some((i:any)=>!i.csi_section_id)?<section className="proposal-print-section"><h3>Unassigned / General Pricing</h3><table className="proposal-price-table"><thead><tr><th>Description</th><th>Qty</th><th>Unit</th><th>Unit Price</th><th>Total</th></tr></thead><tbody>{baseItems.filter((i:any)=>!i.csi_section_id).map((i:any)=><tr key={i.id}><td>{i.description}</td><td>{Number(i.quantity).toLocaleString()}</td><td>{i.unit}</td><td>{money(i.unit_price)}</td><td>{money(Number(i.quantity)*Number(i.unit_price))}</td></tr>)}</tbody></table></section>:null}

    <section className="proposal-print-section"><h3>Base Proposal Summary</h3><table className="proposal-price-table"><tbody>{sections.map((s:any)=>{const subtotal=baseItems.filter((i:any)=>i.csi_section_id===s.id).reduce((sum:number,i:any)=>sum+Number(i.quantity)*Number(i.unit_price),0);return <tr key={s.id}><td>{s.csi_code}</td><td>{s.csi_title}</td><td>{money(subtotal)}</td></tr>})}</tbody><tfoot><tr><td colSpan={2}>TOTAL BASE PROPOSAL</td><td>{money(base)}</td></tr></tfoot></table></section>
    ...
    <section className="proposal-acceptance"><h3>Proposal Acceptance</h3><p>This proposal is offered subject to the scope, pricing basis, qualifications, exclusions, contract documents, and terms stated above. Final contractual obligations are subject to the executed agreement between the parties.</p><div className="proposal-signatures"><div><span>Accepted By / Company</span><i></i><span>Name / Title</span><i></i></div><div><span>Signature</span><i></i><span>Date</span><i></i></div></div></section>

...
export default async function ProposalPrint({params}:{params:Promise<{id:string}>}){
  ...
  const totalEstimatedStHours=laborRates.reduce((s:number,r:any)=>s+Number(r.estimated_st_hours||0),0)
  const totalEstimatedOtHours=laborRates.reduce((s:number,r:any)=>s+Number(r.estimated_ot_hours||0),0)
  const totalEstimatedDtHours=laborRates.reduce((s:number,r:any)=>s+Number(r.estimated_dt_hours||0),0)

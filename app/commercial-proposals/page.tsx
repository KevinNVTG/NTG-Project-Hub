...
export default async function CommercialProposalsPage(){
 ...
 const {data:rows=[]}=await supabase.from('commercial_proposals').select('id,proposal_number,status,proposal_date,valid_until,title,pricing_basis,template_depth,projects(project_number,project_name),customers(first_name,last_name,company_name),commercial_proposal_items(quantity,unit_price,is_alternate)').order('created_at',{ascending:false})

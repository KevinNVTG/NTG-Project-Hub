...
import { formatNtgDateTime } from '@/lib/ntg-date'
...
export default async function ProjectDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: project }, { data: customers }, { data: activity }, { data: estimates }, { data: contracts }, { data: changeOrders }, { data: purchaseOrders }] = await Promise.all([
    supabase.from('projects').select('*, customers(id,first_name,last_name,co_client_first_name,co_client_last_name,company_name,email,phone,co_client_email,co_client_phone,billing_address)').eq('id', id).maybeSingle(),
    supabase.from('customers').select('id,first_name,last_name,co_client_first_name,co_client_last_name,company_name').order('company_name'),
    ...
    supabase.from('change_orders').select('id,change_order_number,status,revision_date,title,amount,contract_id').eq('project_id', id).order('created_at', { ascending: false }),
  ...
  const primaryContract = contracts?.find((c: any) => c.status !== 'void') || contracts?.[0]

  return (
    <AppShell title="Project">
      <div className="page-heading"><div><Link className="eyebrow-link" href="/projects">← Projects</Link><div className="project-title-line"><h2>{project.project_number}</h2><span className={`badge badge-status-${project.status}`}>{project.status.replace('_', ' ')}</span></div><p>{project.project_name} · {customerDisplayName(c)}</p></div><div className="quick-actions-inline"><Link className="secondary-button" href={`/estimates/new?project=${id}`}>+ Estimate</Link><Link className="secondary-button" href="/contracts">Contracts</Link>{primaryContract?.id ? <Link className="secondary-button" href={`/change-orders/new?contract=${primaryContract.id}`}>+ Change Order</Link> : null}<Link className="secondary-button" href={`/purchase-orders/new?project=${id}`}>+ PO</Link><button className="secondary-button" disabled title="Invoices are coming later">+ Invoice</button></div></div>

      <section className="grid-cards project-stats"><div className="card"><div className="stat-label">Contract Value</div><div className="stat-value stat-money">{currency(project.contract_amount)}</div></div><div className="card"><div className="stat-label">Committed PO Cost</div><div className="stat-value stat-money">{currency(committedCost)}</div></div><div className="card"><div className="stat-label">Projected Gross Profit</div><div className="stat-value stat-money">{currency(projectedGrossProfit)}</div></div><div className="card"><div className="stat-label">Customer</div><div className="detail-value">{customerDisplayName(c)}</div></div></section>
      ...
      <section className="two-column">
        ...
        <div className="card activity-card"><h3 className="section-title">Activity</h3>{activity?.length ? <div className="activity-list">{activity.map((item) => <div className="activity-item" key={item.id}><div className="activity-dot" /><div><strong>{item.action}</strong><small>{formatNtgDateTime(item.created_at)}</small></div></div>)}</div> : <div className="empty">Activity will appear as work happens on this project.</div>}</div>
      </section>
    </AppShell>
  )
}

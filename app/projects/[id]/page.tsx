...
export default async function ProjectDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: project }, { data: customers }, { data: activity }] = await Promise.all([
  ...
  return (
    <AppShell title="Project">
      <div className="page-heading"><div><Link className="eyebrow-link" href="/projects">← Projects</Link><div className="project-title-line"><h2>{project.project_number}</h2><span className={`badge badge-status-${project.status}`}>{project.status.replace('_', ' ')}</span></div><p>{project.project_name} · {customerName(c)}</p></div><div className="quick-actions-inline"><button className="secondary-button" disabled title="Estimate Builder is coming in Sprint 2">+ Estimate</button><button className="secondary-button" disabled title="Purchase Orders are coming later">+ PO</button><button className="secondary-button" disabled title="Invoices are coming later">+ Invoice</button></div></div>
      ...
      <div className="project-tabs"><span className="active">Overview</span><span>Documents <small>Coming next</small></span><span>Financials <small>Coming next</small></span><span>Photos <small>Coming later</small></span><span>Activity</span></div>

...
export default async function EstimatePrintPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: estimate } = await supabase.from('estimates').select('*, projects(project_number,project_name,project_address),customers(first_name,last_name,co_client_first_name,co_client_last_name,company_name,email,phone,co_client_email,co_client_phone,billing_address),estimate_items(*)').eq('id', id).maybeSingle()
  ...
  return <main className="estimate-document">
    <section className="document-sheet">
      ...
      <div className="document-meta-grid"><div><span>Prepared For</span><strong>{customerDisplayName(customer)}</strong><p>{customer?.billing_address || project?.project_address || ''}</p><p>{customer?.phone || ''}{customer?.phone && customer?.email ? ' · ' : ''}{customer?.email || ''}</p>{customer?.co_client_phone || customer?.co_client_email ? <p>{customer?.co_client_phone || ''}{customer?.co_client_phone && customer?.co_client_email ? ' · ' : ''}{customer?.co_client_email || ''}</p> : null}</div><div><span>Project</span><strong>{project?.project_name || '—'}</strong><p>{project?.project_address || '—'}</p></div><div><span>Estimate Date</span><strong>{estimate.estimate_date}</strong><span>Valid Until</span><strong>{estimate.valid_until || '—'}</strong></div></div>
      ...
      {estimate.exclusions ? <section className="document-section"><h2>Exclusions</h2><p className="preline">{estimate.exclusions}</p></section> : null}
